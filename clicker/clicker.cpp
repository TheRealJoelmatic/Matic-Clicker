...
void c_clicker::thread()
{
	...
	while ( vars::b_is_running )
	{
		...
		{
			// Left button
			this->b_is_left_clicking = (config.clicker.b_enable_left_clicker && vars::key::clicker_enabled.leftdown());
			if (this->b_is_left_clicking)
				send_click(input::mouse_button_t::left, config.clicker.f_left_cps);

			// Right button
			this->b_is_right_clicking = (config.clicker.b_enable_right_clicker && vars::key::clicker_enabled.rightdown());
